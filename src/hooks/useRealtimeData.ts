...
export function useRealtimeData() {
  const initialPools = getAllPools();
  const [pools, setPools] = useState<Pool[]>(initialPools);
  
  // Calculate initial stats from pools
  const initialTotalNav = initialPools.reduce((sum, pool) => sum + pool.latestNav, 0);
  const initialPreviousNav = initialPools.reduce((sum, pool) => sum + pool.previousNav, 0);
  const initialNavChange = initialPreviousNav > 0 ? ((initialTotalNav - initialPreviousNav) / initialPreviousNav) * 100 : 0;
  
  const [stats, setStats] = useState<RealtimeStats>({
    totalNav: initialTotalNav,
    navChange: initialNavChange,
    activePoolsCount: initialPools.filter(p => p.status === "Healthy").length,
